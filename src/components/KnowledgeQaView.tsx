...
export function KnowledgeQaView({ onOpenSettings, answerer }: KnowledgeQaViewProps) {
  ...
  return (
    ...
    >
      <aside className="z-10 flex max-h-[220px] min-h-0 flex-col border-b border-space-border bg-space-900/[0.8] backdrop-blur-xl md:max-h-none md:border-b-0 md:border-r">
        ...
        <nav aria-label="聊天历史" className="min-h-0 flex-1 overflow-x-auto overflow-y-hidden p-3 md:overflow-x-hidden md:overflow-y-auto md:p-4">
          {conversationGroups.length === 0 ? (
            <p className="px-2 py-4 text-xs leading-6 text-space-muted">尚无历史聊天，第一个问题将自动创建。</p>
          ) : (
            <div className="flex gap-3 md:block md:space-y-5">
              {conversationGroups.map(group => (
                <section key={group.label} className="min-w-[210px] md:min-w-0">
                  <h2 className="mb-2 px-2 font-mono text-[10px] tracking-[0.16em] text-space-faint">{group.label}</h2>
                        ...
                        >
                          {renaming ? (
                            <input
                              ...
                              aria-label="重命名聊天"
                              className="m-1.5 w-[calc(100%-0.75rem)] rounded-lg border border-celadon bg-space-850 px-2.5 py-2 text-xs text-space-text outline-none ring-2 ring-celadon/10"
                            />
                          ) : (
                            <button
                              type="button"
                              ...
                              className="w-full truncate px-3 py-3 pr-16 text-left text-sm text-space-muted"
                          ...
                          {!renaming && (
                            <div className={`absolute right-1.5 top-1/2 flex -translate-y-1/2 items-center rounded-lg bg-inherit ${active ? 'opacity-100' : 'opacity-0 group-hover:opacity-100 group-focus-within:opacity-100'}`}>
                              <button
                                ...
                                className="grid h-8 w-7 place-items-center text-xs text-space-muted hover:text-space-text"
                              >
                                <IconDots />
                              </button>
                              <button
                                ...
                                className="grid h-8 w-7 place-items-center text-base text-space-faint hover:text-cinnabar"
                              >
                                ×
                              </button>
                            </div>
        ...
        <div className="hidden border-t border-space-border px-5 py-4 text-[11px] leading-5 text-space-muted md:block">
          <span className="mr-2 inline-block h-1.5 w-1.5 rounded-full bg-celadon" />
          聊天保存在本机 · 默认全部课件
        </div>
      </aside>

      <main className="flex min-h-0 min-w-0 flex-col bg-space-950/[0.48] backdrop-blur-[2px]">
        <header className="flex items-center justify-between border-b border-space-border bg-space-900/90 px-5 py-4 backdrop-blur md:px-8">
          <div>
            ...
            <h1 data-testid="qa-chat-title" className="mt-1 max-w-[52vw] truncate font-song text-xl font-bold text-space-text">
        ...
        >
          {!initialized || loadingConversation ? (
            <div className="mx-auto mt-16 max-w-xl text-center text-sm text-space-muted">正在打开聊天…</div>
          ) : orderedMessages.length === 0 ? (
            <div className="mx-auto mt-[8vh] max-w-xl text-center">
              <div className="mx-auto grid h-16 w-16 rotate-3 place-items-center rounded-[22px] border border-celadon/25 bg-celadon/10 font-song text-2xl font-bold text-celadon shadow-nebula-panel">问</div>
              <h2 className="mt-7 font-song text-2xl font-bold text-space-text">从全部课件知识卡片开始提问</h2>
              <p className="mx-auto mt-3 max-w-md text-sm leading-7 text-space-muted">我会沿着知识网检索相关卡片，并保留可追溯的课件索引。</p>
            </div>
          ) : (
            <div className="mx-auto max-w-3xl space-y-7">
              {orderedMessages.map(message => (
                <article key={message.id} className={message.role === 'user' ? 'flex justify-end' : ''}>
                  {message.role === 'user' ? (
                    <div className="max-w-[88%] whitespace-pre-wrap rounded-[20px] rounded-tr-[5px] bg-celadon/15 px-5 py-3.5 text-sm leading-7 text-space-text ring-1 ring-celadon/20 md:max-w-[76%]">
                      {message.content}
                    </div>
                  ) : (
                    <div className="w-full border-l-2 border-celadon/35 pl-4 md:pl-6">
                      {message.status === 'pending' && (
                        <div role="status" aria-live="polite" className="flex items-center gap-3 py-3 text-sm text-space-muted">
                      ...
                      {(message.status === 'failed' || message.status === 'interrupted') && (
                        <div className="rounded-2xl border border-cinnabar/20 bg-cinnabar/5 p-4">
                          ...
                          <button
                            ...
                            className="mt-3 rounded-lg border border-cinnabar/25 bg-space-850 px-3 py-2 text-xs font-medium text-cinnabar hover:bg-cinnabar hover:text-white disabled:opacity-40"
                          >
                            重新生成
                          </button>
                        </div>
                      ...
                      {message.citations && message.citations.length > 0 && (
                        <div className="mt-5 flex flex-wrap gap-2 border-t border-space-border pt-4">
                          {message.citations.map((citation, index) => (
                            <button
                              ...
                              className="rounded-lg border border-celadon/25 bg-space-850 px-3 py-2 text-left text-xs text-space-text transition hover:-translate-y-px hover:border-celadon hover:bg-space-750"
                            >
          ...
          <div className="mx-auto max-w-3xl">
            <div className="flex items-end gap-2 rounded-2xl border border-space-border bg-space-850 p-2 shadow-nebula-panel focus-within:border-celadon focus-within:ring-2 focus-within:ring-celadon/10">
              <textarea
                ...
                className="max-h-36 min-h-[44px] min-w-0 flex-1 resize-none bg-transparent px-3 py-2.5 text-sm leading-6 text-space-text outline-none placeholder:text-space-faint"
              />
              <button
                type="button"
            ...
            <p className="mt-2 text-center font-mono text-[9px] tracking-wide text-space-faint">ENTER 发送 · SHIFT + ENTER 换行</p>
          </div>
        </div>
      </main>
...
function CitationDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-space-950/70 backdrop-blur-[2px]" onMouseDown={event => {
      ...
      <aside
        ...
        className="absolute inset-y-0 right-0 flex w-full max-w-[440px] flex-col border-l border-space-border bg-space-900 shadow-[-18px_0_55px_rgba(0,0,0,0.44)]"
      >
        <header className="flex items-start justify-between border-b border-space-border px-6 py-5">
          <div className="min-w-0 pr-4">
            <p className="font-mono text-[9px] tracking-[0.2em] text-cinnabar">CITATION SNAPSHOT</p>
            <h2 id="citation-drawer-title" className="mt-1 font-song text-xl font-bold text-space-text">知识卡片索引</h2>
          </div>
          <button
            type="button"
            ref={closeButtonRef}
            aria-label="关闭引用详情"
            onClick={onClose}
            className="grid h-9 w-9 shrink-0 place-items-center rounded-full border border-space-border text-xl text-space-muted hover:border-celadon hover:text-space-text"
          >
            ×
          </button>
        </header>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-6 py-6">
          ...
          {sourceExcerpt && (
            <section className="mt-6 border-t border-space-border pt-5">
              <p className="font-mono text-[9px] tracking-[0.18em] text-cinnabar">SOURCE EXCERPT</p>
              <h4 className="mt-1 font-song text-base font-bold text-space-text">课件原文摘要</h4>
              <MarkdownRenderer content={sourceExcerpt} className="mt-3 text-xs leading-6 text-space-muted" />
            </section>
          )}
        </div>

        <div className="border-t border-space-border p-5">
          <button
            type="button"
