...
export function MasterNoteView({ onOpenSettings }: { onOpenSettings: () => void }) {
  ...
  const activeCards = activeChapter
    ...
    : [];
  const displayMarkdown = activeChapter?.markdown.trim()
    ? activeChapter.markdown
    : !usableMaster ? outlineMarkdown(plan) : masterNote!.markdown;
          ...
          <aside className="w-72 flex-shrink-0 overflow-y-auto border-r border-stone-200 bg-[#fffdfa] p-3">
            ...
            <nav className="space-y-1">
              {plan.map((chapter, index) => {
                ...
                return (
                  <button key={chapter.id} type="button" onClick={() => setActiveChapterId(chapter.id)} className={`w-full rounded-xl px-3 py-3 text-left transition ${active ? 'bg-[#173f35] text-white shadow-sm' : 'text-stone-700 hover:bg-[#edf4ef]'}`}>
          ...
          <article className="mx-auto max-w-4xl">
            ...
            {activeChapter?.status === 'failed' && (
              <div className="mb-5 rounded-2xl border border-red-200 bg-red-50 p-5">
                <h2 className="font-medium text-red-800">本章生成失败</h2>
                <p className="mt-2 text-sm text-red-700">{activeChapter.error || '模型没有返回有效正文'}</p>
                <button type="button" disabled={isRunning} onClick={() => void retryChapter(activeChapter.id)} className="mt-4 rounded-lg bg-[#173f35] px-4 py-2 text-sm text-white disabled:opacity-50">重试本章</button>
              </div>
            )}
            <div className="rounded-2xl border border-stone-200 bg-[#fffdfa] px-8 py-7 shadow-[0_8px_30px_rgba(23,63,53,.05)]">
              <MarkdownRenderer content={displayMarkdown} className="text-[15px] leading-8" />
            </div>
          </article>
        </main>

        <aside className="w-80 flex-shrink-0 overflow-y-auto border-l border-stone-200 bg-[#fffdfa] p-4">
          <h2 className="text-sm font-bold text-[#173f35]">章节依据</h2>
          <p className="mt-1 text-xs leading-5 text-stone-400">当前章节使用的知识卡片；原始证据仍保留在卡片页。</p>
          <div className="mt-4 space-y-3">
            {activeCards.length > 0 ? activeCards.map(item => (
              ...
              <div className="rounded-xl border border-dashed border-stone-300 p-4 text-xs leading-6 text-stone-400">
                {activeChapter?.status === 'failed' ? '本章尚未成功绑定知识卡片。' : '生成本章后会显示对应知识卡片。'}
              </div>
            )}
          </div>
          {masterNote && (
