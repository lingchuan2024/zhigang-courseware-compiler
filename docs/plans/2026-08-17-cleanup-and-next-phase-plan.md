...
日期：2026-08-17
状态：Phase 0（PR #2）、Phase 1（PR #3）已合并；Phase 2 已完成（分支 `feat/retrieval-quality`：查询改写 + 质量可见化），待合并
...
> **Phase 2 执行结果（2026-08-18）**：7.1 查询改写——一次轻量模型调用把问题转写为 2-3 个"从卡片术语词表
> （主题名/关键词/别名）中选词"的检索查询，复用词法检索管线（多查询按卡片取最高分合并）；无 Key/超时/解析失败
> 均回退原问题，行为零损失；改写词显示在回答下方并计入用量统计（新任务类型 query-rewrite）。顺带修复了检索排序
> 比较器的不一致问题（平分时依赖 V8 排序布局）。7.2 质量可见化——管线校验报告的块覆盖率/主题统计落库为
> structureQuality，知识网头部显示"内容覆盖率 X%"徽章，母笔记章节显示"引用 K 张卡片"角标。7.3 embedding 维持
> 不做，待真实使用反馈。验收用例：与卡片词面零重叠的问题在改写后命中（store 集成 + 纯函数双层测试）。
